...
const CartModal = ({ cart, removeFromCart, closeModal }) => {
    return (
      <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white text-center p-5 rounded-xl w-1/2 max-h-[705px] md:w-1/2 overflow-y-scroll lg:w-2/4 shadow-md">
          ...
          {cart.length === 0 ? (
            ...
            <ul>
              {cart.map(item => (
                <li key={item.id} className="lg:flex-row lg:flex lg:justify-between border-2 border-black shadow-lg rounded-2xl my-7 gap-5">
